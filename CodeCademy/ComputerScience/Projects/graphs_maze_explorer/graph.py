...
class Graph:
  ...
  def explore(self):
    ...
    # while the current room is NOT the treasure room
    while current_room != 'treasure room':
        node = self.graph_dict[current_room]
        # Get the weight for the edges connecting the current room
        ...
        valid_choices = [room[0] for room in node.edges.keys()] #list: index '0' for room in node.edges
        print('\nYou have accumulated: {0} cost'.format(path_total))
        choice = input('\nWhich room do you move to? ')

        if choice not in valid_choices:
            print('please select form these letters: {0}'.format(valid_choices))
        else:
            for room in node.edges.keys():
                if room.startswith(choice): # choice == room[0]
                     current_room = room
                     path_total += node.edges[room]
            print('\n*** You have chosen: {0} ***\n'.format(current_room))
    print('\nMade it to the treasure room with {0} cost'.format(path_total))
